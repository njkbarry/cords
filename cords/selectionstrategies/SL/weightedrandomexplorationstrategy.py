...
class WeightedRandomExplorationStrategy(object):
    ...
    def __init__(
        ...
        self.probs = taylor_softmax_v1(
            torch.from_numpy(np.array([self.global_gains]) / self.temperature)
        ).numpy()[0]
    ...
    def select(self, budget):
        ...
        if self.per_class:
            per_cls_cnt = [
                len(self.cluster_idxs[key]) for key in self.cluster_idxs.keys()
            ]
            ...
            if min_cls_cnt < math.ceil(budget / self.num_classes):
                ...
                per_cls_budget = [
                    math.ceil(budget / self.num_classes) for _ in per_cls_cnt
                ]
            ...
            if self.online:
                ...
                for i in range(len(cluster_labels)):
                    ...
                    sel_idxs = rng.choice(
                        per_cls_idxs,
                        size=per_cls_budget[i],
                        replace=False,
                        p=self.probs[per_cls_idxs] / self.probs[per_cls_idxs].sum(),
                    )
            ...
            if self.online:
                ...
                self.indices = rng.choice(
                    self.global_idxs, size=budget, replace=False, p=self.probs
                )
