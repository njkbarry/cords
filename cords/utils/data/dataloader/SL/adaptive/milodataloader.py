...
class MILODataLoader(AdaptiveDSSDataLoader):
    ...
    def __init__(self, train_loader, dss_args, logger, *args, **kwargs):
        self.strategy = WeightedRandomExplorationStrategy(
            train_loader, dss_args.global_order_file, online=True, temperature=dss_args.temperature, per_class=dss_args.per_class
        )
        super(MILODataLoader, self).__init__(train_loader, train_loader, dss_args, logger, *args, **kwargs)
        """
        ...
        assert (
            "gc_ratio" in dss_args.keys()
        ), "'gc_ratio' is a compulsory argument when warm starting the model(i.e., kappa > 0). Include it as a key in dss_args"
        ...
        dss_args.stochastic_subsets_file = dss_args.stochastic_subsets_file
    ...
    def __iter__(self):
        ...
        if self.cur_epoch < math.ceil(self.gc_ratio * self.num_epochs):
            self.logger.debug("Using GC Stochastic Data Loader from epoch {0:d}".format(self.cur_epoch, math.ceil((1 / 12) * self.num_epochs)))
