...
# from sklearn.datasets import load_boston    # depreciated
...
# FIXME: hack fixe for double up on datasets namespace
...
def gen_dataset(datadir, dset_name, feature, seed=42, isnumpy=False, **kwargs):
    ...
    if dset_name == "dna":
        ...
        x_trn, x_tst, y_trn, y_tst = train_test_split(
            x_trn, y_trn, test_size=0.2, random_state=seed
        )
        ...
        x_trn, x_val, y_trn, y_val = train_test_split(
            x_trn, y_trn, test_size=0.005, random_state=seed
        )
        ...
        fullset = TinyImageNet(
            root=datadir, split="train", download=True, transform=tiny_transform
        )
        testset = TinyImageNet(
            root=datadir, split="val", download=True, transform=tiny_tst_transform
        )
        ...
        clean_type = 0
        fullset = GlueDataset(
            raw["train"], "sentence", "label", clean_type, num_cls, wordvec_dim, wordvec
        )
        ...
        if (
            (os.path.exists(os.path.join(os.path.abspath(data_dir), "trainset.pkl")))
            and (os.path.exists(os.path.join(os.path.abspath(data_dir), "valset.pkl")))
            and (os.path.exists(os.path.join(os.path.abspath(data_dir), "testset.pkl")))
        ):
            trainset = torch.load(
                os.path.join(os.path.abspath(data_dir), "trainset.pkl")
            )
            ...
            torch.save(
                trainset, os.path.join(os.path.abspath(data_dir), "trainset.pkl")
            )
        ...
        clean_type = 1
        fullset = GlueDataset(
            raw["train"], "text", "label", clean_type, num_cls, wordvec_dim, wordvec
        )
        testset = GlueDataset(
            raw["test"], "text", "label", clean_type, num_cls, wordvec_dim, wordvec
        )
        ...
        tokenizer_mapping = lambda example: tokenize_function(
            kwargs["tokenizer"], example, SENTENCE_MAPPINGS["glue_sst2"]
        )
        glue_dataset = glue_dataset.map(tokenizer_mapping, batched=True)
        glue_dataset = glue_dataset.remove_columns(
            [SENTENCE_MAPPINGS["glue_sst2"], "idx"]
        )
        ...
        tokenizer_mapping = lambda example: tokenize_function(
            kwargs["tokenizer"], example, SENTENCE_MAPPINGS["hf_trec6"]
        )
        trec6_dataset = trec6_dataset.map(tokenizer_mapping, batched=True)
        trec6_dataset = trec6_dataset.remove_columns(
            [SENTENCE_MAPPINGS["hf_trec6"], "fine_label"]
        )
        trec6_dataset = trec6_dataset.rename_column(
            LABEL_MAPPINGS["hf_trec6"], "labels"
        )
        ...
        tokenizer_mapping = lambda example: tokenize_function(
            kwargs["tokenizer"], example, SENTENCE_MAPPINGS["imdb"]
        )
        ...
        tokenizer_mapping = lambda example: tokenize_function(
            kwargs["tokenizer"], example, SENTENCE_MAPPINGS["rotten_tomatoes"]
        )
        ...
        tokenizer_mapping = lambda example: tokenize_function(
            kwargs["tokenizer"], example, SENTENCE_MAPPINGS["tweet_eval"]
        )
        ...
        tweet_dataset = tweet_dataset.rename_column(
            LABEL_MAPPINGS["tweet_eval"], "labels"
        )
